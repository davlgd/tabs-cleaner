...
- Counts for windows, tabs, pinned and audible tabs — Pinned and Audible are clickable and jump to the first matching tab
...
- Per-URL close (trash icon), per-domain "Close all" (multi-URL domains only), and a bulk **Clean up duplicates** action on the filtered set
...
Only `tabs`. No `host_permissions`, no content script, no storage, no network request, no background service worker.
...
- Preferences aren't persisted: filter, collapsed sections and scroll reset each time the dashboard is opened.
- When many tabs change in the same millisecond, a render may briefly show stale counts; the next refresh corrects it.
